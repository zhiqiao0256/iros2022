...
class pc_client(object):
    ...
    def __init__(self):
        ...
        self.tau_x_y_array=np.array([0.0]*2)
        """ Thearding Setup """
        self.th1_flag=True
        self.th2_flag=True
        self.flag_reset=1
    ...
    def th_sub_pub_mocap(self):# thread config of read data from mocap and send packed msg to record file.
        try:
            while self.run_event.is_set() and self.th2_flag:
                ...
                self.array2setsRecord=np.concatenate((self.pd_pm_array, self.array2setswithrotation, self.tau_array, self.state_variable_array,self.tau_theta_phi_array,self.tau_x_y_array), axis=None)
                if self.flag_reset==0:
                    self.send_zipped_socket1(self.array2setsRecord)
                    print "t:",round(self.t_new,2),"pd/pm:",np.around(self.pd_pm_array,1),"tau:",np.around(self.tau_x_y_array,2)
                # sleep(0.005)
            exit()
        except KeyboardInterrupt:
            exit()
    ...
    def openloopTorqueCtrl(self):
            ...
            self.sine_response_torque_x_y(phi,theta)
            self.x1_old=self.x1_current
            self.x3_old=self.x3_current
            self.t_old=self.t_new
    ...
    def sine_response_torque_x_y(self,phi,theta):
        #[t_x,t_y]'=T_matrix_1*[d_p12,d_p13]
        #d_p21=p2-p1, d_p13= p1-p3
        #t_x=0.5 * (p1 + p3) - p2= -1.0* p21 -0.5 * p13
        #t_y = sqrt(3)/2(p1-p3)= sqrt(3)/2* p13
        ...
        sphi= np.sin(phi)
        ### Map from tau_theta, phi to tau_x, y
        self.tau_x_y_array=A_array*np.sin(2.0*np.pi*freq_array*self.t_old)+ B_array
        tau_x= self.tau_x_y_array[0]
        tau_y = self.tau_x_y_array[1]
        ### Map from tau_x,y to p1,p2,p3 MPa tau_y,x = Q_mat * trans([p,p13])
        inv_Q_11=1.0
        inv_Q_12=1.0/np.sqrt(3)
        inv_Q_21=0.
        inv_Q_22=-2.0/np.sqrt(3)
