...
%Azadeh version created 03/11/22
%Bending for grasping-Silicone
%Bias in pressure for experiment only to avoid the dead zone
...
pressure_of_tubes=[1;zeros(STEPS,1)];
...
vstar = [ 0 ; 0 ; 1.033];    % initial position velocity w.r.t arc length (local frame)
ustar = [ 0 ; 0.2283 ; 0 ];   % initial curvature for a straight rod (rotation derivative w.r.t arc length) (local frame)
    ...
    %% main original code solving the coesserat rod forward dynamics
     ...
     if size(mocap_data)>0
        ...
        if abs(position_local(1))<=0.00000001
            ...
            kapa(2) = -(1)*2*position_local(1)/(position_local(1)^2+position_local(3)^2);  %curvature_about_x_axis
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1) = 1+MPa_to_Psi*Pressure_from_actuator_L_1(i);
...
%         lext = cross(fext,[0;0;0]);   %external moment to the segment (constant and global)
       %% PD control-shear -part of solving inverse dynamics- global frame- where we close the loop
        ...
        ud(i,:,node+1) = u_d;
        vd(i,:,node+1) = v_d;
        if v_d(3) < 1.033  %1.0212
            f_total = [0;0;0];
        else
            f_total = 25*R*(Kse*(v_d-v) + Bse*(vt_d-vt)); 
        end
        if u_d(2)< 0.2283  %1.1096
            l_total = [0;0;0];
        else
...
%              if segment==1
        ...
        %          
                 ...
                 v_zd = 1+0.08*sin(frequency*t(i))^2; %/STEPS; 
                 vt_zd = 0.08*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %/STEPS;
                 vtt_zd = 0.08*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2); 
