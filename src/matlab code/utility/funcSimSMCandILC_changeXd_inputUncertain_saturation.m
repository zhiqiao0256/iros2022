function par_set=funcSimSMCandILC_changeXd_inputUncertain_saturation(par_set)
close all
%% Input Output data
...
% %%%% Step input
          seed = rng;
          numOfSetps = deg2rad(-10+(-40-(-10))*rand(1,4));
...
max_iteration=200;
kp=1.0;
e_k=zeros(length(timeArray),max_iteration);
x_k=e_k;
...
for iteration=1:max_iteration
    ...
    for i=2:length(timeArray)-1
        ...
        e(i,1)=xd_aug(i,iteration)-x1(i,1);
        de(i,1)=(e(i)-e(i-1))/Ts;
        ...
        sum_of_ek(i,iteration)=kp*sum(e_k(i,:));
        eta=1/(1+B_star)*(B_star*abs(lambda*de(i,1)+ddxd(i,1) - f)+Km*abs(x1(i,1)/M)+Dm*abs(x2(i,1)/M)+epsi+abs(sum_of_ek(i,iteration)));
        u_alpha(i,1)=M/alpha*(lambda*de(i,1)+ddxd(i,1) - f+ eta*sat+sum_of_ek(i,iteration));
        %     %%% Input Signal bound
        if u_alpha(i,1)>=uMax
