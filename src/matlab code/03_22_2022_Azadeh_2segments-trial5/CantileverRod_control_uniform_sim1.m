...
%Improving 2 segments bending simulation
...
function output= CantileverRod_control_uniform_sim1
...
pressure_of_tubes = [1,1,0,0;zeros(STEPS-1,N*2)];
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,4) = MPa_to_Psi*Pressure_from_actuator(i,1); %positive y
        pressure_of_tubes(i+1,3) = MPa_to_Psi*Pressure_from_actuator(i,2); %negative y
        pressure_of_tubes(i+1,2) = MPa_to_Psi*Pressure_from_actuator(i,3); %negative x
        pressure_of_tubes(i+1,1) = MPa_to_Psi*Pressure_from_actuator(i,4); %positive x
...
%             end
            ...
            l_total(i,:,segment) = 25*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut)); 
            lenv(i,:) = cross(R*ds*e3,fenv(i,:));
