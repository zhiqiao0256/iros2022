...
global t N exp_result sim_result segment vd ud kapa_out L_out lenv lseg fseg pressure_of_tubes
%%
figure()
plot(t(1:end),100*exp_result(1:end,1,N+1))
hold on;
plot(t,100*sim_result(:,1,N+1))
xlabel('T(s)');ylabel('X(cm)')
title('position in x-axis')
%%
figure()
plot(t, 100*exp_result(:,2,N+1))
hold on;
plot(t, 100*sim_result(:,2,N+1))
xlabel('T(s)');ylabel('Y(cm)')
title('position in y-axis')
%%
figure()
plot(t, 100*exp_result(:,3,N+1))
hold on;
plot(t, 100*sim_result(:,3,N+1))
...
plot(t,ud(:,2,N+1))
hold on;
plot(t,sim_result(:,24,N+1))
hold on;
plot(t,exp_result(:,24,N+1))
hold on;
plot(t,kapa_out(:,N+1))
hold on;
plot(t,exp_result(:,24,N+1)-sim_result(:,24,N+1))
title('curvature vs. desired value vs. error in between')
%%
figure()
plot(t,vd(:,3,N+1))
hold on;
plot(t,sim_result(:,22,N+1))
hold on;
plot(t,exp_result(:,22,N+1))
hold on;
plot(t,exp_result(:,22,N+1)-sim_result(:,22,N+1))
