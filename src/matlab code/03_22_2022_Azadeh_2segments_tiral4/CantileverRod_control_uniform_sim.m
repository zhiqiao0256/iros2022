...
%Azadeh version created 03/22/22
...
%Trying 2 segments bending simulation
...
function output= CantileverRod_control_uniform_sim
%% Parameters - Section 2.1
global elapsedTime dL kapa L f_total l_total fseg lseg rhoAg ds C q t sim_result exp_result ud vd N kapa_out L_out Pressure_from_actuator pressure_of_tubes fenv lenv
% N L STEPS 
%% Video recording ()
...
f_total = zeros(STEPS,3,N+1);
l_total = zeros(STEPS,3,N+1);
...
r_segment_1_positive = 0.03*cos(pi/4)*2*-e2;   %sum of r1+r2 since P1A1=P2A2 seg1 [m]
r_segment_1_negative = 0.03*cos(pi/4)*2*e2;    %sum of r3+r4 since P3A3=P4A4 seg1 [m]
r_segment_2_positive = 0.03*cos(pi/4)*2*-e1;    %sum of r1+r2 since P1A1=P2A2 seg2 [m]
r_segment_2_negative = 0.03*cos(pi/4)*2*e1;    %sum of r3+r4 since P3A3=P4A4 seg2 [m]
%% Cross-section area
...
% y_bias_base = -0.2160;
% z_bias_base = -0.0148;
% R_base_initial = [ 0.999994894425506	0.00282945208288510	0.00148503327615934
%           -0.00283053837013910	0.999995727640934	0.000729898906744846
%           -0.00148296171758197	-0.000734098623860707	0.999998630960940];
% 
% x_bias_segment_1 = 6.9800e-04-0.1195;
% y_bias_segment_1 = 1.100e-05+0.2160;
% z_bias_segment_1 = 0.1742+0.0148;
% R_seg1_initial  = [ 0.999988247023763	0.00482293658756138	-0.000495072736738072
%           -0.00484189743762402	0.998689345076428	-0.0509524097567496
%            0.000248683625977892	0.0509542080057039	0.998700959668592];
% 
% x_bias_segment_2 = 0.0024;
% y_bias_segment_2 = 0.0072;
% z_bias_segment_2 = 0.3726;
% R_seg2_initial  = [ 0.999977539521966	0.00649614655896355	-0.00164940337093299
%           -0.00649176281125072	0.999975418638040	0.00264936507100044
%            0.00166657349014112	-0.00263859802953029	0.999995130154763];
%%
for i = 1:STEPS
    i
    tic;
...
%        position_global_base = mocap_data(1:3)-[x_bias_base,y_bias_base,z_bias_base];
%        position_global_segment_1 = mocap_data(8:10)-[x_bias_segment_1,y_bias_segment_1,0]-position_global_base;
%        position_global_segment_2 = mocap_data(15:17)-[x_bias_segment_2,y_bias_segment_2,0]-position_global_base;
% 
...
%             kapa(3)=0;
%             dL(3)=0;
%             L(3) = position_local_segment_2(3);
%         else       
%             %curvature_about_y_axis 
%             kapa(3) = -(1)*2*position_local_segment_2(2)/(position_local_segment_2(2)^2+position_local_segment_2(3)^2);              
%             L(3) = (2/kapa(3))*atan2(-position_local_segment_2(2),position_local_segment_2(3));
%             dL(3) = L(3)-L0/N;
%         end
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,4) = 1+MPa_to_Psi*Pressure_from_actuator(i,1)*8; %positive y
        pressure_of_tubes(i+1,3) = 1+MPa_to_Psi*Pressure_from_actuator(i,2)*8; %negative y
        pressure_of_tubes(i+1,2) = 1+MPa_to_Psi*Pressure_from_actuator(i,3)*8; %negative x
        pressure_of_tubes(i+1,1) = 1+MPa_to_Psi*Pressure_from_actuator(i,4)*8; %positive x
...
%                 ds=L0_2;
%             end
            f_total(i,:,segment) = (25)*R*(Kse*(v_d-v) + Bse*(vt_d-vt)); 
            fenv(i,:) = rhoAg ;  

            l_total(i,:,segment) = 80*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut)); 
            lenv(i,:) = cross(R*ds*e3,fenv(i,:));

%             fseg(i,:,segment) = [0;0;f_total(i,3,segment)] - fenv(i,:)'; 
%             lseg(i,:,segment) = [0;l_total(i,2,segment);0] - lenv(i,:)'; 
...
%             Pressure_from_actuator(i,:) = direction_of_segment_moment'*fseg(i,:,segment)'/(4*A_chamber);
        ...
        if segment==2
             fseg(i,:,segment) = [0;0;f_total(i,3,segment)] - fenv(i,:)'; 
             lseg(i,:,segment) = [l_total(i,1,segment);0;0] - lenv(i,:)'; 
             if lseg(i,1,segment) >= 0
                 r2=r_segment_1_negative;
...
%% calculating the input to the segment for the next loop considering all dynamics 
        ns = R*rhoA*(cross(w,q)+qt)  - fenv(i,:)' ;%- fseg(i,:,segment)' - fenv(i,:)'; 
        ms = R*(cross(w,rhoJ*w) + rhoJ*wt) - cross(ps,n) - 8*lseg(i,:,segment)';%-lenv(i,:)';        
        %compatibility
...
%% Grasp related input definition, switching conditions based on sensor feedback 
    function grasp(segment)
        ...
        if segment==2
%                  if i<=50
%                      u_gain= 2; %6/physical_segment;
%                      u_bias= 0.25;
%                      u_xd =  u_bias+(u_gain-u_bias)*sin(frequency*t(i))^2; %exp(alpha*t(i)); %
%                      ut_xd = (u_gain-u_bias)*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %alpha*exp(alpha*t(i));
%                      utt_xd = (u_gain-u_bias)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
% 
%                  else
%                      u_gain= -2; %6/physical_segment;
%                      u_bias= 0.25;
%                      u_xd =  u_bias+(u_gain+u_bias)*sin(frequency*t(i-50))^2; %exp(alpha*t(i)); %
%                      ut_xd = (u_gain+u_bias)*2*frequency*cos(frequency*t(i-50))*sin(frequency*t(i-50)); %alpha*exp(alpha*t(i));
%                      utt_xd = (u_gain+u_bias)*2*frequency^2*(cos(frequency*t(i-50))^2-sin(frequency*t(i-50))^2);%alpha*alpha*exp(alpha*t(i));
% 
%                  end
%                  v_gain = 1;
...
%                  utt_xd = (u_gain-u_bias)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
        ...
        elseif segment==1
                 if i<=50
                     u_gain= 1; %6/physical_segment;
                     u_bias= 0.025;
                     u_yd =  u_bias+(u_gain-u_bias)*sin(frequency*t(i))^2; %exp(alpha*t(i)); %
                     ut_yd = (u_gain-u_bias)*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %alpha*exp(alpha*t(i));
                     utt_yd = (u_gain-u_bias)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));

                 else
                     u_gain= -1; %6/physical_segment;
                     u_bias= 0.025;
                     u_yd =  u_bias+(u_gain+u_bias)*sin(frequency*t(i-50))^2; %exp(alpha*t(i)); %
                     ut_yd = (u_gain+u_bias)*2*frequency*cos(frequency*t(i-50))*sin(frequency*t(i-50)); %alpha*exp(alpha*t(i));
                     utt_yd = (u_gain+u_bias)*2*frequency^2*(cos(frequency*t(i-50))^2-sin(frequency*t(i-50))^2);%alpha*alpha*exp(alpha*t(i));
