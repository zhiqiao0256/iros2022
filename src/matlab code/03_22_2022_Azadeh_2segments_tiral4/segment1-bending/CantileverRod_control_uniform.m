...
% N L STEPS 
%% Video recording ()
myVideo = VideoWriter('Bending-segment1'); %open video file
...
bias_base = [0.1196;-0.2170;-0.0153];
R_base_initial = [ 0.9999    0.0028   -0.0141
                  -0.0026    0.9999    0.0137
                   0.0142   -0.0137    0.9998];

bias_segment_1 = [0.1229;-0.2173;-0.0153]; %0.1612

R_seg1_initial  = [-0.0112    0.9998    0.0147
                   -0.9999   -0.0113    0.0069
                    0.0071   -0.0146    0.9999];

bias_segment_2 = [0.1239;-0.2210;-0.0153]; %0.3569

R_seg2_initial  = [-0.0124    0.9999    0.0111
                   -0.9998   -0.0126    0.0121
                    0.0122   -0.0110    0.9999];
%%
for i = 1:STEPS
    i
    tic;
...
%             h2*h4 - h3*h1 , h3*h4 + h2*h1 , -h2^2 - h3^2];
       ...
       position_local_segment_2 = (R_seg2_initial'*R_seg2)'*position_global_segment_2';
        ...
        if abs(position_local_segment_1(1))<=0.00000001
            ...
            kapa(2) = -2*position_local_segment_1(1)/(position_local_segment_1(1)^2+position_local_segment_1(3)^2);              
        ...
        if abs(position_local_segment_2(2))<=0.00000001
            ...
            %curvature_about_y_axis 
            kapa(3) = 2*position_local_segment_2(2)/(position_local_segment_2(2)^2+position_local_segment_2(3)^2);              
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,4) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,1); %positive y
        pressure_of_tubes(i+1,3) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,2); %negative y
        pressure_of_tubes(i+1,2) = 1+MPa_to_Psi*Pressure_from_actuator(i,3); %positive x
        pressure_of_tubes(i+1,1) = 1+MPa_to_Psi*Pressure_from_actuator(i,4); %negative x
