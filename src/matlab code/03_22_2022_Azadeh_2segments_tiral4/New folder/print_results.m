...
close all;
clear all;
output = CantileverRod_control_uniform_sim;
...
%%
figure()
plot(0.000145038*Pressure_from_actuator)
hold on;
plot(pressure_of_tubes(2:end,4))
...
plot(ud(:,2,N))
hold on;
plot(exp_result(2:end,24,N))
hold on;
plot(sim_result(:,24,N),'.-')
title('Curvature')
...
plot(ud(:,1,N+1))
hold on;
plot(exp_result(2:end,23,N+1))
hold on;
plot(sim_result(:,23,N+1),'.-')
title('Curvature')
legend('Desired','Experiment','Simulation')
%%
% figure()
