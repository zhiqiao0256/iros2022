...
%Azadeh version created 03/10/22
...
%      0.0170,0,0.1868,0.9966,0,0.0827,0,0,0,0,0,0,0,0.0170,0,0.0026,0,0.1145,0.0]';
% z = [0,0,1,0,0,0;0,0,1.0212,0,1.1096,0]';
y_prev = y;
z_prev = z;
...
%         lseg(i,:,segment) = 30*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut))-lenv(i,:)'+(pressure_initial*A_chamber*direction_of_segment_moment);  
        if v_d(3) < 1.0212 %1.004
            f_total = [0;0;0];
        else
         f_total = 15*R*(Kse*(v_d-v) + Bse*(vt_d-vt)); 
        end
        if u_d(2)< 1.1096  %0.32
            l_total = [0;0;0];
        else
