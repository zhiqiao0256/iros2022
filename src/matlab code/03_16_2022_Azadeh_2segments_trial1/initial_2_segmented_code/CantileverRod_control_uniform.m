...
%Azadeh version created 03/16/22
...
global elapsedTime dL kapa L fseg lseg rhoAg ds C q t sim_result exp_result ud vd segment N kapa_out L_out Pressure_from_actuator pressure_of_tubes fenv lenv
% N L STEPS 
%% Video recording ()
...
pressure_of_tubes= zeros(STEPS,N*2);
...
% r1 = 0.03*(cos(pi/4))*(e1+e2);  %actuator chamber 1 distance to backbone vector  [m]
% r2 = 0.03*(cos(pi/4))*(e1-e2);  %actuator chamber 2 distance to backbone vector  [m]
% r3 = 0.03*(cos(pi/4))*(e1+e2);  %actuator chamber 3 distance to backbone vector  [m]
% r4 = 0.03*(cos(pi/4))*(e1-e2);  %actuator chamber 4 distance to backbone vector  [m]
r_segment_1_positive = -0.03*cos(pi/4)*2*e1;   %sum of r1+r2 since P1A1=P2A2 seg1
r_segment_1_negative = 0.03*cos(pi/4)*2*e1;    %sum of r3+r4 since P3A3=P4A4 seg1
r_segment_2_positive= -0.03*cos(pi/4)*2*e2;    %sum of r1+r2 since P1A1=P2A2 seg2
r_segment_2_negative = 0.03*cos(pi/4)*2*e2;    %sum of r3+r4 since P3A3=P4A4 seg2
%% Cross-section area
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,1) = MPa_to_Psi*Pressure_from_actuator(i,1); %negative x
        pressure_of_tubes(i+1,2) = MPa_to_Psi*Pressure_from_actuator(i,2); %positive x
        pressure_of_tubes(i+1,3) = MPa_to_Psi*Pressure_from_actuator(i,3); %positive y
        pressure_of_tubes(i+1,4) = MPa_to_Psi*Pressure_from_actuator(i,4); %negative y
...
%         lext = cross(fext,[0;0;0]);   %external moment to the segment (constant and global)
       %% PD control-shear -part of solving inverse dynamics- global frame- where we close the loop
        ...
        vd(i,:,node+1) = v_d;
            ...
            f_total = 40*R*(Kse*(v_d-v) + Bse*(vt_d-vt)); 
            fenv(i,:) = rhoAg ;  
        ...
        elseif segment==2
             fseg(i,:,segment) = [0;0;f_total(3)] - fenv(i,:)'; 
             lseg(i,:,segment) = [0;l_total(1);0] - lenv(i,:)'; 
             if l_total(1) <=0
                 r=r_segment_2_positive;
                 direction_of_segment_moment = 2*cross(p,R*e3)+cross(R*r,R*e3);
                 Pressure_from_actuator(i,3) = direction_of_segment_moment'*lseg(i,:,segment)'/(A_chamber);
             else
                 r=r_segment_2_negative;
                 direction_of_segment_moment = 2*cross(p,R*e3)+cross(R*r,R*e3);
                 Pressure_from_actuator(i,4) = direction_of_segment_moment'*lseg(i,:,segment)'/(A_chamber);
             end
        end
%         pressure_initial = (0.7/MPa_to_Psi);
%         lseg(i,:,segment) = 30*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut))-lenv(i,:)'+(pressure_initial*A_chamber*direction_of_segment_moment);  
%         direction_of_segment_moment = 2*cross(p,R*e3)+cross(R*r,R*e3);
%         Pressure_from_actuator_L_1(i) = direction_of_segment_moment'*lseg(i,:,segment)'/(A_chamber);

%         direction_of_segment_force = R*e3;   %*PiAi=fp = fseg            
...
%                  u_yd =  0.25+(alpha-0.25)*(sin(frequency*t(i)));
%                  ut_yd =  (alpha-0.25)*(frequency)*cos(frequency*t(i));
%                  utt_yd =
%                  -(alpha-0.25)*(frequency)^2*sin(frequency*t(i));
                 if i< 50              
...
%                  u_yd =  0.25+(alpha-0.25)*(sin(frequency*t(i)));
%                  ut_yd =  (alpha-0.25)*(frequency)*cos(frequency*t(i));
%                  utt_yd = -(alpha-0.25)*(frequency)^2*sin(frequency*t(i));
                 
                 u_xd = 0;% 0.25+(alpha-0.25)*sin(frequency*t(i))^2; %exp(alpha*t(i)); %
                 ut_xd = 0;% (alpha-0.25)*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %alpha*exp(alpha*t(i));
                 ...
                 v_zd = 1;%1.0033+(0.08-0.0033)*sin(frequency*t(i))^2; %/STEPS; 
                 vt_zd = 0;%(0.08-0.0033)*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %/STEPS;
                 vtt_zd = 0;%(0.08-0.0033)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2); 
%
             end
