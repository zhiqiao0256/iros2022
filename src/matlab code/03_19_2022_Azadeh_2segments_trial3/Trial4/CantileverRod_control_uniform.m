...
% N L STEPS 
%% Video recording ()
myVideo = VideoWriter('Extension-seg1'); %open video file
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,3) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,3)/75; %positive y
        pressure_of_tubes(i+1,4) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,4)/75; %negative y
...
%         lext = cross(fext,[0;0;0]);   %external moment to the segment (constant and global)
       %% PD control-shear -part of solving inverse dynamics- global frame- where we close the loop
        ...
        vd(i,:,node+1) = v_d;
            ...
            Pressure_from_actuator(i,1) = direction_of_segment_moment'*fseg(i,:,1)'/(4*A_chamber);
            Pressure_from_actuator(i,2) = direction_of_segment_moment'*fseg(i,:,1)'/(4*A_chamber);
            Pressure_from_actuator(i,3) = direction_of_segment_moment'*fseg(i,:,2)'/(4*A_chamber);
            Pressure_from_actuator(i,4) = direction_of_segment_moment'*fseg(i,:,2)'/(4*A_chamber);
%         if segment==1
%              fseg(i,:,segment) = [0;0;f_total(3)] - fenv(i,:)'; 
%              lseg(i,:,segment) = [0;l_total(2);0] - lenv(i,:)'; 
%              if l_total(2) >=0
...
%                  utt_xd = 0;%(alpha-0.25)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
                 ...
                 v_zd = 1;%+(0.1)*sin(frequency*t(i))^2; %/STEPS; 
                 vt_zd = 0;%(0.1)*2*frequency*cos(frequency*t(i))*sin(frequency*t(i));
                 vtt_zd = 0;%(0.1)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2); 
%                  v_zd = 1;
%                  vt_zd = 0;
%                  vtt_zd = 0;
%                  v_zd = 1.0033+(0.08-0.0033)*sin(frequency*t(i))^2; %/STEPS; 
