...
%Azadeh version created 03/08/22
%Bending and extension
%Non-zero state to start the simulation with experiment- done:)
...
global elapsedTime L dL fseg lseg rhoAg ds C q t sim_result exp_result ud vd segment N kapa_out L_out Pressure_from_actuator_L_1 pressure_of_tubes Pressure_from_actuator_L_1 fenv lenv
% N L STEPS 
%% Video recording ()
...
vstar = [ 0 ; 0 ; 1.004 ];    % initial position velocity w.r.t arc length (local frame)
ustar = [ 0 ; 0.32 ; 0 ];   % initial curvature for a straight rod (rotation derivative w.r.t arc length) (local frame)
...
% output = load('output');
y = [0,0,0,1,0,0,0,0.0015,-0.0,50.6324,0.0,1.8613,0,0,0,0,0,0,0;
     0.0170,0.0,0.1868,0.9966,0.0,0.0827,0,0,0,0,0,0,0,0.0170,0,0.0026,0,0.1145,0.0]';
z = [0,0,1,0,0,0;0,0,1.004,0,0.32,0]';
y_prev = y;
z_prev = z;
...
%         plotRobot();
        ...
        sim_result(i,:,:) = [y;z]; 
        plotRobot();
        pressure_of_tubes(i) = MPa_to_Psi*Pressure_from_actuator_L_1(i);
...
%         initial_guess(i,:) = G;
        ...
        for j = 1:N
            ...
            for counter = 1:length(t_c)
                ...
                circle(counter,:) = [x_c;y_c;z_c]; 
                p_c(counter,:,1) = [r0*cos(t_c(counter));r0*sin(t_c(counter));0];
                p_c(counter,:,j+1) = R*y(1:3,j+1)+R*circle(counter,:)';
            end
...
%         direction_of_segment_force = R*e3;   %*PiAi=fp = fseg 
        direction_of_segment_moment = 2*cross(p,R*e3)+cross(R*r,R*e3); %*PiAi=lp = lseg
...
%         lseg(i,:,segment) = 30*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut))-lenv(i,:)'+(pressure_initial*A_chamber*direction_of_segment_moment);  
        if v_d(3) < 1.004 %1.0212
            f_total = [0;0;0];
        else
         f_total = 15*R*(Kse*(v_d-v) + Bse*(vt_d-vt)); 
        end
        if u_d(2)< 0.32  %1.1096
            l_total = [0;0;0];
        else
         l_total = 30*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut)); 
        end
    ...
    function grasp()
        ...
        frequency = 2*pi/T_period; 
             ...
             if segment==1
                 ...
                 u_xd =  0; %alpha*sin(frequency*t(i))^2; %exp(alpha*t(i)); %
                 ut_xd = 0; %alpha*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %alpha*exp(alpha*t(i));
                 utt_xd = 0; %alpha*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
        %          
                 u_yd =  alpha*sin(frequency*t(i))^2; %exp(alpha*t(i)); %
                 ut_yd = alpha*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %alpha*exp(alpha*t(i));
                 utt_yd = alpha*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
                 ...
                 v_zd = 1+0.07*sin(frequency*t(i))^2; %/STEPS; 
                 vt_zd = 0.07*2*frequency*cos(frequency*t(i))*sin(frequency*t(i)); %/STEPS;
                 vtt_zd = 0.07*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2); 
