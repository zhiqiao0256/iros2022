...
%Azadeh version created 04/25/22
...
% N L STEPS 
%% Video recording ()
% myVideo = VideoWriter('Bending-both'); %open video file
...
L0_1 = 0.1791;%0.1613+0.0153;
L0_2 = 0.3769-L0_1;%0.3572+0.0153-L0_1;
L0 = L0_1+L0_2;
...
pressure_of_tubes = [0,0,0,1;zeros(STEPS,N*2)];
...
recv = tcpserver("10.203.49.210",6666)
sen = tcpclient("10.203.48.122",7777)
datasize = 21;

bias_base = [0.3199;-0.1368;-0.0161];
R_base_initial = [ 1.0000    0.0006    0.0003
                  -0.0006    1.0000    0.0003
                  -0.0003   -0.0003    1.0000];

bias_segment_1 = [0.3185;-0.1309;-0.0161]; %0.1617

R_seg1_initial  = [1.0000    0.0014    0.0008
                  -0.0014    1.0000   -0.0031
                  -0.0008    0.0031    1.0000];

bias_segment_2 = [0.3284;-0.1247;-0.0161]; %0.3566

R_seg2_initial  = [1.0000    0.0008    0.0012
                  -0.0008    1.0000   -0.0022
                  -0.0012    0.0022    1.0000];
%%
for i = 1:STEPS
    i
    tic;
...
%             h2*h4 - h3*h1 , h3*h4 + h2*h1 , -h2^2 - h3^2];
       ...
       position_local_segment_2 = (R_seg2_initial'*R_seg2)'*position_global_segment_2';
        ...
        if abs(position_local_segment_1(2))<=0.00000001
            kapa(2)=0;
            dL(2)=0;
            L(2) = position_local_segment_1(3);
        else       
            kapa(2) = -(1)*2*position_local_segment_1(2)/(position_local_segment_1(2)^2+position_local_segment_1(3)^2);              
            L(2) = (2/kapa(2))*atan2(-position_local_segment_1(2),position_local_segment_1(3));
            dL(2) = L(2)-L0_1;
        end
         %curvature_about_x_axis 
        if abs(position_local_segment_2(1))<=0.00000001
            kapa(3)=0;
            dL(3)=0;
            L(3) = position_local_segment_2(3)-position_local_segment_1(3);
        else       
            %curvature_about_y_axis 
            kapa(3) = -(1)*2*position_local_segment_2(1)/(position_local_segment_2(1)^2+position_local_segment_2(3)^2);              
            L(3) = (2/kapa(3))*atan2(position_local_segment_2(1),position_local_segment_2(3))-L(2);
            dL(3) = L(3)-L0_2;
        end
        ...
        z(4,2) = kapa(2);   %ux changes for segment 1
        z(5,3) = kapa(3);   %uy changes for segment 2
...
%         plotRobot();
        ...
        pressure_of_tubes(i+1,4) = 1+MPa_to_Psi*Pressure_from_actuator(i,4); %negative x
        pressure_of_tubes(i+1,3) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,3)*3; %positive x
        pressure_of_tubes(i+1,2) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,2); %negative y
        pressure_of_tubes(i+1,1) = 0;%1+MPa_to_Psi*Pressure_from_actuator(i,1)*3; %positive y
...
%         lext = cross(fext,[0;0;0]);   %external moment to the segment (constant and global)
       %% PD control-shear -part of solving inverse dynamics- global frame- where we close the loop
        ...
        vd(i,:,node+1) = v_d;
            ...
            l_total(i,:,segment) = 15*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut)); 
            lenv(i,:) = cross(R*ds*e3,fenv(i,:));
...
%% calculating the input to the segment for the next loop considering all dynamics 
        ...
        ms = R*(cross(w,rhoJ*w) + rhoJ*wt) - cross(ps,n) - (1/4)*lseg(i,:,segment)';%-lenv(i,:)';        
        %compatibility
...
%% Grasp related input definition, switching conditions based on sensor feedback 
    function grasp(segment)
      ...
      if segment==1
%                  if i<=50
%                      u_gain= 10; %6/physical_segment;
%                      u_bias= 0;%-0.1031;%-0.547;
...
%                  else
%                      u_gain= 0.75; %6/physical_segment;
%                      u_bias= -0.1031;%-0.547;
...
%                  utt_xd = (u_gain-u_bias)*2*frequency^2*(cos(frequency*t(i))^2-sin(frequency*t(i))^2);%alpha*alpha*exp(alpha*t(i));
     ...
     elseif segment==2
%                  if i<=50
                     u_gain= -3; %6/physical_segment;
                     u_bias= -0.0273; %-0.2318;%-0.547 ;% 0;%.0273;  %
