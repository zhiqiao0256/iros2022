...
%Azadeh version created 03/02/22
...
global R L dL fseg lseg rhoAg ds C q t result ud vd segment N kapa_out L_out pressure_of_tubes Pressure_from_actuator_L_1 lenv
% N L STEPS 
%% Video recording ()
...
%         read(t_tcp,datasaver,'double');
        position_global(i,:,:) = y_exp(i,:,:);
        position(1) = position_global(i,1,2); 
        position(2) = position_global(i,2,2); 
        position(3) = position_global(i,3,2); 
%         position_local = position;
        position_local = (R'*position')';
...
%         result(i,:,:) = [y;z]; 
        plotRobot();
        pressure_of_tubes(i) = 30*0.000145038*Pressure_from_actuator_L_1(i) ;
        write(t_tcp,pressure_of_tubes(i));
    end
...
%         lext = cross(fext,[0;0;0]);   %external moment to the segment (constant and global)
       %% PD control-shear -part of solving inverse dynamics- global frame- where we close the loop
        ...
        lseg(i,:,segment) = 30*R*(Kbt*(u_d-u) + Bbt*(ut_d-ut))-lenv(i,:)';   %summation of the moments that is required to be applied to the backbbone from the actutors
